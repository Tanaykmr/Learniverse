...
import { isCourseLoading, coursePrice, courseTitle, courseDescription, courseImage } from "../../store/selectors/Course.js";
...
function GrayTopper() {
  console.log("hello from graytopper, title is: ", courseState.title)
  const title = useRecoilValue(courseState).title;
  return (
    <div
      style={{
        height: 250,
...
function UpdateCard() {
  ...
  // i tried using currentCourse = useRecoilValue(courseDetails, and title, desc etc as currentCourse.title etc, but it does not work due to async nature and it taking time to get a value
  ...
  useEffect(() => {
    if (currentCourse) {
      setTitle(useRecoilValue(courseTitle));
      setDescription(courseDescription);
      setImageLink(courseImage);
      setPrice(coursePrice);
      setIsChecked(coursePublished);
    }
  }, [currentCourse]);
